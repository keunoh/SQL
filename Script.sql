...
--------------------------- 조인튜닝 56번 ---------------------------------- 
/** 아래 데이터 모델에서 접수일자가 오늘인 개통접수 및 장애접수 데이터를 모두 출력하는 SQL을 작성하고, 최적인덱스를 설계
...
SELECT A.작업일련번호
 ...
 WHERE B.개통접수번호 = DECODE(A.작업구분코드, 'A', A.접수번호)
   AND C.장애접수번호 = DECODE(A.작업구분코드, 'B', A.접수번호)
   AND B.개통접수일시 = TO_DATE(TO_CHAR(SYSDATE, 'YYYYMMDD'), 'YYYYMMDD') 
   AND C.장애접수일시 = TO_DATE(TO_CHAR(SYSDATE, 'YYYYMMDD'), 'YYYYMMDD')

/** 해설
 * 제시한 배타적 관계 모델에서 조회 조건이 작업지시 테이블에 있으면, 상황에 따라 OUTER 조인, UNION ALL 방식을 선택할 수 있다.
 * 반면, 조회 조건이 개통접수 및 장애접수 테이블에 있고 NL 조인으로 처리해야 유리한 소량의 결과집합이라면, 선택의 여지가 없다.
 * 무조건 UNION ALL 방식이 유리하다. 
 * 개통접수 및 장애접수 테이블이 드라이빙 되도록 조인 순서를 유도하고, 그에 최적화된 인덱스 구성을 해 주면 된다.
 * */
   
-- 모범답안 [ SQL ] 
SELECT Y.작업일련번호, Y.작업자ID, '개통' AS 작업구분, X.고객번호, X.주소
  FROM 개통접수 X, 작업지시 Y
 WHERE X.개통접수번호 = Y.접수번호
   AND X.개통접수일시 >= TRUNC(SYSDATE)
   AND X.개통접수일시 < TRUNC(SYSDATE + 1)
   AND Y.작업구분코드 = 'A'
 UNION ALL
SELECT Y.작업일련번호, Y.작업자ID, '장애' AS 작업구분, X.고객번호, X.주소
  FROM 장애접수 X, 작업지시 Y
 WHERE X.장애접수번호 = Y.접수번호
   AND X.장애접수일시 >= TRUNC(SYSDATE)
...
--------------------------- 조인튜닝 58번 ---------------------------------- 
/** 아래 데이터 모델에서 장비구분코드가 'A001'인 장비의 최종상태코드와 직전일의(=상태변경이력의 변경일자가 장비의 최종상태변경일자보다 작은)
 * 마지막 상태코드, 변경일자, 변경순번을 출력하는 최적 SQL을 작성
 * - 장비구분코드 = 'A001'인 장비는 10건
 * - 결과집합을 장비번호 순으로 정렬
 * - DBMS는 오라클이며, 12C 이후 버전임
 * */ 
...
-- 내 답안
SELECT 
	   B.장비번호, B.장비명, B.최종상태코드
	 , A.직전상태코드, A.직전변경일자, A.직전변경순번
  FROM (
  	SELECT 장비번호, 변경일자, 변경순번, 상태코드 
  	  FROM (
		SELECT /*+ INDEX_DESC(H 상태변경이력_PK) */ *
  	  	  FROM 상태변경이력 H
  	 	 ORDER BY 변경일자 DESC, 변경순번 DESC  	  
  	  )
  	 WHERE ROWNUM = 2
...
/** ROWID를 이용한 모범답안 2안은 상태변경이력_PK 인덱스를 한 번만 액세스하므로 약간 더 유리하긴 하지만,  
 * 일반 개발자가 이해하기 쉽지 않은 패턴이어서 권장하고 싶지는 않다.
 * 힌트를 사용하지 않았지만, ORDER BY 조건에 따라 인덱스를 역순으로 스캔한 사실을 확인하자.
 * 아래와 같이 ORDER BY를 생략하고 INDEX_DESC 힌트를 명시하면 SQL은 좀 더 단순해지지만,
 * 혹시라도 인덱스 구성이 변경되면 결과집합에 오류가 발생하므로 가급적 사용하지 않는 것이 좋다.
 * (모범답안처럼 서브쿼리 내 인라인 뷰에서 메인쿼리 컬럼을 참조하면 ORA-00904 에러가 발생하던 과거 버전에서 어쩔수 없이 사용하던 방식이다.)
 * */ 
...
SELECT P.장비번호, P.장비명, P.최종상태코드, H.상태코드 AS 직전상태코드
  ...
  FROM 장비 P, 상태변경이력 H
 WHERE P.장비번호 = H.장비번호
--   AND P.장비구분코드 = 'A001' -> 이 조인절은 없어도 무방
   AND H.ROWID = 
 ...
 * 		TABLE ACCESS (BY USER ROWID) OF '상태변경이력'
 * 		  COUNT (STOPKEY)
 * 			INDEX (RANGE SCAN DESCEDING) OF '상태변경이력_PK'
 * */ 
...
--------------------------- 조인튜닝 59번 ---------------------------------- 
/** 아래 데이터 모델에서 장비구분코드가 'A001'인 장비의 최종(=현재) 상태코드와 변경일자를 출력하는 최적 SQL
 * - 장비구분코드 = 'A001'인 장비는 10건
 * - 결과집합을 장비번호 순으로 정렬
 * - 장비 테이블의 최종변경일시는 상태코드 변경과 무관
 * - 상태변경이력은 선분이력 테이블
 * - 장비 테이블의 최종변경일시와 상태변경이력의 유효시작일시, 유효종료일시는 DATE 형
 * */ 
/** [ 인덱스 구성 ]
 * 장비_PK : [ 장비번호 ]
 * 장비_X1 : [ 장비구분코드 ]
...
SELECT
	   X.장비번호
	 , (SELECT P2.장비명 FROM 장비 P2 WHERE P2.장비번호 = X.장비번호) AS 장비명
	 , X.상태코드
	 , (SELECT MAX(최종변경일시) FROM 장비 P2 WHERE P2.장비번호 = X.장비번호) AS 최종상태변경일자
  FROM (
  	SELECT H.장비번호
  		 , H.상태코드
  	  FROM 상태변경이력 H
  	 WHERE SYSDATE H.유효시작일시 AND H.유효종료일시
  	   AND H.장비번호 = (SELECT P.장비번호 
  	   				  FROM 장비 P 
  	   				 WHERE P.장비구분코드 = 'A001')
  ) X   
 
/** 해설
 * 앞서 푼 문제에서 상태변경이력은 '점이력' 모델이었다. 즉, 변경일자와 변경순번만 관리하는 방식이다.
 * 본 문제의 상태변경이력은 '선분이력' 모델이다. 즉, 변경이 발생한 시점뿐만 아니라 해당이력이 유효했던 종료 시점을 같이 관리하는 방식이다.
 * 이 모델의 장점은 무엇보다 쿼리가 단순하다는 데 있다. 쿼리가 단순해지면 성능도 그만큼 좋아진다.
 ...
 *   AND 유효종료일시 >= SYSDATE
 * 
 * 선분이력에서 최종 데이터의 유효종료일시에는 9999년 12월 31일 23시 59분 59초를 입력하므로 아래와 같이 '=' 조건절을 사용할 수도 있다.
 * 
 * WHERE 유효종료일시 = TO_DATE('99991231 235959', 'YYYYMMDD HH24MISS')
 * */ 
...
--------------------------- 조인튜닝 60번 ---------------------------------- 
/** 아래 데이터 모델에서 장비구분코드가 'A001'인 장비의 최종 상태코드와 변경일자, 
 * 그리고 직전(=상태변경이력의 유효시작일시가 장비의 최종상태변경일시보다 작은) 상태변경이력의 상태코드와
 * 변경일자를 출력하는 최적 SQL
 * - 장비구분코드 = 'A001'인 장비는 10건
 * - 결과집합을 장비번호 순으로 정렬
 * - 장비 테이블의 최종상태변경일시는 상태코드를 가장 마지막으로 변경한 일시
 * - 상태변경이력은 선분이력 테이블
 * - 장비 테이블의 최종상태변경일시와 상태변경이력의 유효시작일시, 유효종료일시는 DATE 형
 * */ 
/** [ 인덱스 구성 ]
 * 장비_PK : [ 장비번호 ]
 * 장비_X1 : [ 장비구분코드 ]
...
SELECT P.장비번호, P.최종상태코드
 ...
 WHERE P.장비번호 = H.장비번호
   ...
   AND (H.유효시작일시, H.유효종료일시) = 
   	     (SELECT 유효시작일시, 유효종료일시
   	     	FROM (SELECT H2유효시작일시, H2.유효종료일시 
					FROM 상태변경이력 H2
				   WHERE H2.장비번호 = P.장비번호
					 AND H2.유효시작일시 < P.최종상태변경일시
				   ORDER BY H2.유효종료일시 DESC, H2.유효시작일시 DESC  
				  )
		   WHERE ROWNUM <= 1
   	     )
   
/**해설
 * 본 문제의 상태변경이력은 '선분이력' 모델이다. 즉, 변경이 발생한 시점뿐만 아니라 해당 이력이 유효했던 종료 시점을 같이 관리하는 방식이다.
 * 선분이력에서 현재가 아닌 특정 시점 이력을 조회할 때는 '=' 조건을 사용할 수 없으며, 아래와 같이 BETWEEN 조건을 사용해야 한다.
 * 
 * WHERE :기준시점 BETWEEN 유효시작일시 AND 유효종료일시
 * 
 * BETWEEN 조건을 분해하면 아래와 같다.
 * 
 * WHERE 유효시작일시 <= :기준시점
 *   AND 유효종료일시 >= :기준시점
 * 
 * 본 문제에서는 직전 이력, 즉 유효시작일시가 장비의 최종상태변경일시보다 작은 이력을 찾아야 하므로 조인 조건을 아래와 같이 작성해야 한다.
 * 
 * WHERE 상태변경이력.장비번호 =  장비.장비번호
 *   AND 상태변경이력.유효시작일시 <  장비.최종상태변경일시
 *   AND 상태변경이력.유효종료싱릿 >= 장비.최종상태변경일시 - 1/(60*60*24)
 * */
  
-- 모범답안 [ SQL ]
SELECT P.장비번호, P.장비명, P.최종상태코드
 	 , TO_CHAR(P.최종상태변경일시, 'YYYYMMDD') AD 최종변경일자
 	 , H.상태코드 AS 직전상태코드
...
/**해설
 * 상품유형코드의 선택도가 10%(=100/1,000)이므로 최근 1년간 계약 건수 500만 건 중 50만건을 읽을 것으로 예상된다.
 * DISTINCT 연산을 통해 중복을 제거한 최종 결과집합은 100개의 상품 집합인데, 문제에서 제시한 실행계획대로 실행하면 랜덤 액세스 방식으로 50만 건의 계약 데이터를 읽어야 한다.
 * SQL을 아래 모범답안처럼 EXISTS 서브쿼리로 변환하면, 100개 상품에 대해 계약과 조인에 성공하는 데이터가 존재하는지 여부만 확인하므로 훨씬 효율적이다.
 * 계약일자 조건을 확인하기 위한 테이블 액세스를 피하려면, 계약_X2 인덱스를 [ 상품번호 + 계약일자 ]로 구성해주는 것이 좋다.
 * 참고로, 계약_X1 인덱스에 상품번호를 추가해도 테이블 액세스를 피할 수 있지만, 인덱스 스캔 비효율에 문제가 생긴다.
 * 
 * FILTER
 * 	  NESTED LOOPS (SEMI)
 * 		TABLE ACCESS (BY INDEX ROWID) OF '상품' (TABLE)
 * 		  INDEX (RANGE SCAN) OF '상품_X1' (INDEX)
 * 		INDEX (RAGNE SCAN) OF '계약_X2' (INDEX)
 * 
 * EXISTS 서브쿼리로 변환했는데 옵티마이저가 서브쿼리를 Unnesting 한 후에 세미 조인하거나 계약 테이블을 먼저 읽어서 상품과 조인하는 방식으로 수행하면 원하는 성능 효과를 얻을 수 없다.
 * 항상 위 실행계획처럼 상품을 먼저 읽어서 계약과 NL 세미 조인하도록 강제 하려면, LEADING과 NL_SJ 힌트를 추가하면 된다.
 * 서브쿼리를 Unnesting 하지 못하도록 NO_UNNEST만 추가해도 된다.
 * 서브쿼리를 Unnesting 하지 않으면 항상 필터 방식으로 실행되기 때문이다.
 * 필터 방식은 NL 세미 조인과 사실상 차이가 없고, 항상 메인 쿼리 집합을 먼저 읽으므로 LEADING 또는 ORDERED 힌트도 불필요하다.
 * 
 * FILTER
 *   TABLE ACCESS (BY INDEX ROWID) OF '상품' (TABLE)
 * 	   INDEX (RANGE SCAN) OF '상품_X1' (INDEX)
